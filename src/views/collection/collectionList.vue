            ...
            <div class="collectionWrap mb30">
                <div class="collection comWidth" style="padding-top:10px;">
                    <div class="titleBar mb-8 bb-e9e9e9">
                        <div class="comTitle">
                            <a href="javascript:;"><span class="titleIcon"></span><strong class="titleName">{{data.name}}</strong></a>
                        </div>
                    </div>
                    <!-- 筛选框 -->
                    <div class="selectBox" style="margin-bottom:20px;">
                        <ul class="select-tabs" id="ul1">
                            <li v-for="(item,index) in tabtitle" :key="index" :class="{active:num ==index}" @click="getAll(item.code,index)">{{item.name}}</li>
                        </ul>
                    </div>
                    <collection-list :isPage="isPage" :data="dataList" :type="type"></collection-list>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
      //请求分类列表
      getMater(){
          let url='master/code/'+this.$route.query.master
      ...
      getAll(code,index){
          ...
          if(this.$route.query.type==1){
              var url="collection/page/"+this.$route.query.number;
          }else {
              var url="specimen/page/"+this.$route.query.number;
          }
          if(code){
              let data ={
                lang:this.lang,
              ...
              API.get2(url,data).then(res => {
                  if (res.code == 0) {
                    ...
                    //console.log(res,456454545454545)
...
.selectBox .select-tabs li.active,
.selectBox .select-tabs li:hover {
