<template>
  ...
  <div class="news_main" style="margin-bootom:40px;">
		...
		<div class="navPath clear">
			<p class="clear"><i></i><a href="/intangible">非遗</a><i></i></p><p class="clear"><i></i>
      <router-link :to="{path:'intangibleLaws'}">非遗法规</router-link><i></i></p>
			<p class="clear">
			<i></i>
				<a href="/intangible/intangibleTypeLaws?type=1">
					国家级法规</a>
        	<i></i>
			</p>
		</div>
...
export default {
  ...
  methods: {
    getDetail(){
           ...
           // console.log(data)
            API.get2('intangible/regulat/get',data).then(res => {
                if (res.code == 0) {
                    this.data = res.data;
